...
export default function ClientCases() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-black text-black tracking-tight">Мои дела</h1>
        ...
        <div className="space-y-4">
          {cases.map(c => (
            <div key={c.id}
              ...
              className="bg-white rounded-2xl border border-black/5 p-5 cursor-pointer hover:border-[#1a0a6e]/20 hover:shadow-sm transition-all">
              <div className="flex items-start justify-between gap-3 mb-4">
                ...
                <Icon name="ChevronRight" size={18} className="text-black/20 shrink-0 mt-1" />
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
                {c.plaintiff && (
                  <div>
                    ...
                    <div className="text-[12px] text-black/70 truncate">{c.plaintiff}</div>
                  </div>
                )}
                {c.defendant && (
                  <div>
                    <div className="text-[10px] font-semibold text-black/35 uppercase tracking-wide mb-0.5">Ответчик</div>
                    <div className="text-[12px] text-black/70 truncate">{c.defendant}</div>
                  </div>
                )}
                {c.amount != null && (
                  <div>
                    <div className="text-[10px] font-semibold text-black/35 uppercase tracking-wide mb-0.5">Сумма иска</div>
                    <div className="text-[13px] font-bold text-[#1a0a6e]">{formatMoney(c.amount)}</div>
                  </div>
                )}
              </div>
              ...
              {c.statuses.length > 0 && (
                <div className="flex items-center gap-1 mt-2">
                  {c.statuses.map((s, i) => (
                    <div key={i} className={`h-1.5 flex-1 rounded-full ${i === c.statuses.length - 1 ? "bg-[#1a0a6e]" : "bg-[#1a0a6e]/25"}`} />
                  ))}
                  <span className="text-[10px] text-black/35 ml-2 shrink-0">{formatDate(c.statuses[c.statuses.length - 1]?.happened_at)}</span>
                </div>
              )}
            </div>
          ))}
